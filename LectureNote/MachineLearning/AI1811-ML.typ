...
$
  P(x) = sum^K_(k=1) alpha_k cal(N) (x|Theta^((k))) = sum^K_(k=1) alpha_k cal(N) (x|mu_k, sum_k)
$
...
#definition("Cov")[
  $
    sum = mat("Cov"(X_1, X_1), "Cov"(X_1, X_2), dots, "Cov"(X_1, X_D); "Cov"(X_2, X_1), "Cov"(X_2, X_2), dots, "Cov"(X_2, X_D);dots.v,dots.v,dots.down,dots.v;
    "Cov"(X_D, X_1),"Cov"(X_D, X_2),dots,"Cov"(X_D, X_D))
  $

  协方差矩阵本身保证对称并且半正定。

  - 协方差矩阵的特征值 $lambda_i$ 决定簇的方差和形状
  - 特征向量决定了椭球体的主轴方向

  协方差矩阵实际上刻画了高斯分布在高维数据下是一个椭球分布。（例如，在二维情况，即只有两个特征值的情况下，是一个椭圆），而特征值就是高维椭球的每一个维度半轴的长度，而具体的特征向量的指向则决定了具体的角度。（而且这个是确定的！因为不同特征向量之间是正交的关系）
]
