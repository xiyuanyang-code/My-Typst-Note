...
class DifferenceConstraintSystem:
    ...
    def __init__(self):
        ...
        # Node definitions (0 is the virtual source)
    ...
    def build_graph(self):
        ...
        # Time window constraints: add virtual source point
    ...
    def bellman_ford(self, source):
        ...
        # Detect negative cycle: perform one more iteration, if still can update then negative cycle exists
        for (u, v, w) in self.edges:
            if dist[u] != math.inf and dist[v] > dist[u] + w:
    ...
    def print_solution(self, dist, neg_cycle):
        ...
        if neg_cycle:
            ...
            print("✅ System is feasible. Following is a set of arrival times (in minutes) that satisfy the constraints:")
            for i in sorted(self.locations):
                print(f"  {self.locations[i]}: x_{i} = {dist[i]:.1f}  (allowed range {self.time_windows[i]})")

            # Check each travel constraint slack
    ...
    def _plot_timeline(self, ax, dist, plt):
        ...
        ax.set_title('Optimal arrival times at each point')
    ...
    def _plot_constraint_graph(self, ax, dist, plt, nx):
        ...
        # Add edges (only show some important edges to avoid overly complex graph)
        ...
        travel_edges = list(self.travel.items())[:6]  # Only show first 6 travel constraints
        for (i, j), t in travel_edges:
            G.add_edge(j, i, weight=-t, color='red')
        
        # Node position layout
        pos = {
            0: (0, 0),  # Virtual source point at center
            1: (-1, 1),  # A
            2: (1, 1),   # B
            3: (-1, -1), # C
            4: (1, -1)   # D
        ...
        ax.set_title('Difference constraint system graph (partial constraints)')
        ax.axis('off')
